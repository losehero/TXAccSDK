...
-(void)loadSelectDateView{
    ...
    self.selectDateView = [[[NSBundle bundleForClass:self.classForCoder]loadNibNamed:@"SelectDateView" owner:self options:nil]lastObject];
...
-(void)doneButton{
    NSString *time = [NSString stringWithFormat:@"%td:%td",self.selectDateView.choseHour,self.selectDateView.choseMinute];
    [self.pickerDelegate datePickerReturn:time];
    [self removeSubViews];
}
