...
typedef NS_ENUM(NSUInteger, MMNumberKeyboardButton) {
    ...
    MMNumberKeyboardButtonSubtract,
...
- (void)_commonInit
{
    ...
    UIButton *specialButton = [_MMNumberKeyboardButton keyboardButtonWithStyle:MMNumberKeyboardButtonStyleWhite];
    [specialButton setTitle:@"+"forState:UIControlStateNormal];
    [buttonDictionary setObject:specialButton forKey:@(MMNumberKeyboardButtonAdd)];
    
    UIButton *subtractButton = [_MMNumberKeyboardButton keyboardButtonWithStyle:MMNumberKeyboardButtonStyleWhite];
    [subtractButton setTitle:@"-"forState:UIControlStateNormal];
    [buttonDictionary setObject:subtractButton forKey:@(MMNumberKeyboardButtonSubtract)];
...
- (void)updateTextWithField:(NSString *)text
{
...
- (void)_buttonInput:(UIButton *)button
{
    ...
    else if(keyboardButton == MMNumberKeyboardButtonSubtract)
    {
        NSString *decimalText = [button titleForState:UIControlStateNormal];
        if ([delegate respondsToSelector:@selector(numberKeyboard:shouldInsertText:)]) {
            BOOL shouldInsert = [delegate numberKeyboard:self shouldInsertText:decimalText];
    ...
    // Handle done.
        ...
        {
            if ([delegate respondsToSelector:@selector(numberKeyboardEqualReturn:)])
            {
                [delegate numberKeyboardEqualReturn:self];
            }
        }
        else
        {
    ...
    // Handle .
        ...
        if ([delegate respondsToSelector:@selector(numberKeyboardWithText:insertText:)])
        {
            decimalText  = [delegate numberKeyboardWithText:self insertText:decimalText];
        }
...
- (NSString *)defaultEqualReturnTitle {
    return @"=";
}
...
- (void)layoutSubviews
{
    ...
    const int utilityButtonKeys[3] = { MMNumberKeyboardButtonAdd,MMNumberKeyboardButtonSubtract,MMNumberKeyboardButtonDone };
    const CGSize utilitySize = CGSizeMake(columnWidth,MMNumberKeyboardRowHeight);
    ...
    for (NSInteger idx = 0; idx < sizeof(utilityButtonKeys) / sizeof(int); idx++) {
        ...
        if (idx != (sizeof(utilityButtonKeys) / sizeof(int)) - 1)
        {
            rect = (CGRect){ .size = utilitySize };
        }
        else
        {
            rect = (CGRect){ .size = CGSizeMake(columnWidth, MMNumberKeyboardRowHeight*2) };
        }
...
- (void)_buttonStyleDidChange
{
    ...
    if (style == MMNumberKeyboardButtonStyleWhite) {
        ...
        fillColor = [UIColor colorWithRed:252.0/255.0 green:217.0/255.0 blue:82.0/255.0 alpha:1];
        highlightedFillColor = [UIColor colorWithRed:252.0/255.0 green:217.0/255.0 blue:82.0/255.0 alpha:1];
    }
